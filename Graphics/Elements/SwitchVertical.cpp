...
//------------------------------------------------------------------------
// HESwitchVertical Implémentation
// ----------------------------

//------------------------------------------------------------------------
HESwitchVertical::HESwitchVertical (VSTGUI::CRect &frameRect, CControlListener * listener, int tag, int nbrPosition, bool _isRound)
: safepadding(frameRect.getWidth()/20)
, ecartementInterieur(3)
, roundRectRadius(5)
, nbrOfChoices(nbrPosition)
, isRound(_isRound)
, CVerticalSwitch (frameRect,
                   listener,
                   tag,
                   nbrPosition,
                   0,
                   nbrPosition,
                   new CBitmap(getWidth(),getHeight())//new CBitmap(getWidth(), getHeight())
                   )
{
    // par sécurité
    getBackground()->forget();
    setBackground(new CBitmap(getWidth(),getHeight()));
    heightOfOneImage = getHeight();
...
//------------------------------------------------------------------------
HESwitchVertical::~HESwitchVertical()
{
...
//------------------------------------------------------------------------
VSTGUI::CMouseEventResult HESwitchVertical::onMouseDown (VSTGUI::CPoint& where, const VSTGUI::CButtonState& buttons)
{
    return VSTGUI::CVerticalSwitch::onMouseDown(where, buttons);
}

//------------------------------------------------------------------------
VSTGUI::CMouseEventResult HESwitchVertical::onMouseMoved (VSTGUI::CPoint& where, const VSTGUI::CButtonState& buttons)
{
    return VSTGUI::CVerticalSwitch::onMouseMoved(where, buttons);
}

//------------------------------------------------------------------------
VSTGUI::CMouseEventResult HESwitchVertical::onMouseUp (VSTGUI::CPoint& where, const VSTGUI::CButtonState& buttons)
{
     return VSTGUI::CVerticalSwitch::onMouseUp(where, buttons);
}
...
//------------------------------------------------------------------------
void HESwitchVertical::draw (VSTGUI::CDrawContext *pContext)
{
    //CVerticalSwitch::draw(pContext);
    ...
    // Dessin de l'handle
    // ------------------
    HESwitchVertical::drawHandle(pContext);
...
//------------------------------------------------------------------------
void HESwitchVertical::drawViewStrokes (VSTGUI::CDrawContext * pContext)
{
...
//------------------------------------------------------------------------
void HESwitchVertical::drawBackground (VSTGUI::CDrawContext * pContext)
{
    pContext->setDrawMode(VSTGUI::kAntiAliasing);
    // début du dessin
...
//------------------------------------------------------------------------
void HESwitchVertical::drawHandle (VSTGUI::CDrawContext * pContext)
{
    double choiceZone = getHeight()-2*safepadding; // = zone de background
    double choiceHeight = choiceZone/(nbrOfChoices) - 3; // la zone de background est divié en nombre de zone de position
    double ecartementEntrePosition = (choiceZone - nbrOfChoices*choiceHeight)/(nbrOfChoices+1);
    ...
    if (isRound) {
        gradRect = graphicHelp::makeCRect(getWidth()/2,
                                          choiceHeight/2 + safepadding + ecartementEntrePosition,
                                          getWidth() - 2*safepadding - 3*ecartementInterieur,
                                          getWidth() - 2*safepadding - 3*ecartementInterieur);
        gradRect.offset(getViewSize().left, getViewSize().top);
        
        slideShadowRect = graphicHelp::makeCRect(getWidth()/2,
                                                 choiceHeight/2 + safepadding + ecartementEntrePosition,
        ...
        handleRect = graphicHelp::makeCRect(getWidth()/2,
                                            choiceHeight/2 + safepadding + ecartementEntrePosition,
                                            getWidth() - 2*safepadding - 5*ecartementInterieur,
                                            getWidth() - 2*safepadding - 5*ecartementInterieur);
    ...
    else
    {
        ...
        gradRect = graphicHelp::makeCRect(getWidth()/2,
                                          ...
                                          choiceHeight-2*ecartementInterieur);
        gradRect.offset(getViewSize().left, getViewSize().top);
        ...
        handleRect = graphicHelp::makeCRect(getWidth()/2,
                                            ...
                                            choiceHeight - 4*ecartementInterieur);
        handleRect.offset(getViewSize().left, getViewSize().top);
    }
    ...
    for (int i = 0; i < nbrOfChoices; i++)
    {
        ...
        // ------------------------
        double decalageHorizontal = choiceHeight + ecartementEntrePosition;
        
        gradRect.offset(0, decalageHorizontal);
        slideShadowRect.offset(0, decalageHorizontal);
        handleRect.offset(0, decalageHorizontal);
